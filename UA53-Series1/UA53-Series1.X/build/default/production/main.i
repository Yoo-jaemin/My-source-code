...
const unsigned int FlashSignatureWord __at(0x2006) = 0x600D;
...
{
if (g_led_status.isBlink == 1 && g_usedtime_count > 18000) {
g_led_status.isBlink = 0;
g_led_status.seq = LED_ALLOFF;

}

if (g_usedtime_count > 360000) {
g_usedtime_count = 0;
memcpy(g_flash_temp.byte, g_nvm.flash.byte, 72);
g_nvm.flash.usedtime++;
if (nvm_write_flash_all(&g_nvm.flash) == 0) {
memcpy(g_nvm.flash.byte, g_flash_temp.byte, 72);
}
if (g_nvm.flash.usedtime >= 17520) {
g_led_status.twoYear = 1;
}
}
}
...
{
if (USBDeviceState == CONFIGURED_STATE) g_usb_state = USB_ATTACHED;
...
sensMeasure_task();
usedtime_task();
if (g_sens_autoCalib.slpSet_en == ON && g_led_status.isBlink == 0)
sens_autoCalib_slp_set_task();
if (g_nvm.flash.auto_Calib == ON && g_led_status.isBlink == 0)
sens_autoCalib_task();
}
}
