...
g_nvm.eeprm[channel].window_size = 2;
...
_strclr(g_nvm.eeprm[channel].sensor_name, 10);
}

static void nvm_clear_flash(void)
{
g_nvm.flash.sensing_on = SENSING_OFF;

g_nvm.flash.ctrl_master_mode = MODBUS_SLAVE_MODE;
g_nvm.flash.ctrl_channel = 0;
g_nvm.flash.ctrl_slave_id = 0x01;
g_nvm.flash.ctrl_relay_no = 1;
g_nvm.flash.ctrl_ppm_max = 15.0f;
g_nvm.flash.ctrl_holding_min = 1;

g_nvm.flash.byte_swap = 0;
g_nvm.flash.slave_id = 54;
g_nvm.flash.baudrate = 5;
g_nvm.flash.timeout = 0;
...
} else if (sensor_model == H2S_50) {
g_nvm.eeprm[chno].sensor_model = H2S_50;
g_nvm.eeprm[chno].adc_gain = FSR_0p512V;
g_nvm.eeprm[chno].lmp_gain = R2p75K;
g_nvm.eeprm[chno].lmp_rload = R100;
g_nvm.eeprm[chno].lmp_intZ = S20P;
_strcpy(g_nvm.eeprm[chno].sensor_name, "H2S-50");
} else if (sensor_model == EO) {
...
_strclr(g_nvm.eeprm[chno].sensor_name, 10);
}
}
...
{
if (!(g_nvm.flash.baudrate >= BAUD_4800 && g_nvm.flash.baudrate <= BAUD_57600)) return 0;
else if (!(g_nvm.flash.ctrl_master_mode >= MODBUS_SLAVE_MODE && g_nvm.flash.ctrl_master_mode <= MODBUS_MASTER_MODE)) return 0;
else if (!(g_nvm.flash.ctrl_channel >= 0 && g_nvm.flash.ctrl_channel <= (2-1))) return 0;
else if (!(g_nvm.flash.ctrl_slave_id >= 0x01 && g_nvm.flash.ctrl_slave_id <= 0x0f)) return 0;
else if (!(g_nvm.flash.ctrl_relay_no >= 1 && g_nvm.flash.ctrl_relay_no <= 8)) return 0;
else if (!(g_nvm.flash.timeout >= TO_ONE_SEC && g_nvm.flash.timeout <= TO_TWO_SEC)) return 0;
else if (!(g_nvm.flash.byte_swap >= 0 && g_nvm.flash.byte_swap <= 1)) return 0;
else if (g_nvm.flash.buildtime == 0) return 0;
return 1;
}
...
bool nvm_write_eeprom_all(uint8_t channel)
{
g_nvm.eeprm[channel].check_sum = checksum(g_nvm.eeprm[channel].byte, 62-2);

uint8_t size = (62 / 8);
for (uint8_t i = 0 ; i < size ; i++) {
if (eeprom_writePage(channel, 0x00 + (i * 8), &g_nvm.eeprm[channel].byte[(i * 8)], 8) == 0)
return 0;
_delay((unsigned long)((10)*(48000000/4000.0)));
}
uint8_t left = 62 - (size * 8);
if (left > 0) {
if (eeprom_writePage(channel, 0x00 + (size * 8), &g_nvm.eeprm[channel].byte[(size * 8)], left) == 0)
return 0;
_delay((unsigned long)((10)*(48000000/4000.0)));
}
return 1;
...
bool nvm_read_eeprom_all(uint8_t channel)
{
return eeprom_RandomRead(channel, 0x00, g_nvm.eeprm[channel].byte, 62);
}

# 270
bool nvm_write_flash_all(u_Flash_Nvm* flash)
{
flash_clear(0xf400, (0xf400 + 30));
return flash_writeBytes(0xf400, flash->byte, 30);
}

void nvm_read_flash_all(u_Flash_Nvm* flash)
{
flash_readBytes(0xf400, flash->byte, 30);
}
