...
if ( requestParser != (0) && requestParser->_base.address == 0 )
{
...
# 54
if ( 5 * sizeof( uint8_t ) > 23 ) return MODBUS_ERROR_ALLOC;
...
if ( status->request.length < 4u || status->request.frame == (0) )
{
...
if ( parser->_base.address != status->address && parser->_base.address != 0 )
...
if ( status->userFunctions != (0) )
{
uint16_t i;
for ( i = 0; i < status->userFunctionCount; i++ )
{
if ( status->userFunctions[i].function == parser->_base.function )
{
functionMatch = 1;


if ( status->userFunctions[i].handler != (0) )
{
...
{
if ( functionMatch )
err = modbusBuildExceptionErr( status, parser->_base.function, MODBUS_EXCEP_ILLEGAL_FUNCTION, MODBUS_FERROR_NULLFUN );
else
err = modbusBuildExceptionErr( status, parser->_base.function, MODBUS_EXCEP_ILLEGAL_FUNCTION, MODBUS_FERROR_NOFUN );
}
...
memset( status->response.frame, 0, 23 );
...
if ( status->registerCallback == (0) ) status->registerCount = status->inputRegisterCount = 0;

# 269
if ( status->coilCount == 0 || status->coils == (0) )
{
status->coilCount = 0;
status->coils = (0);
}

if ( status->discreteInputCount == 0 || status->discreteInputs == (0) )
{
status->discreteInputCount = 0;
status->discreteInputs = (0);
}
...
ModbusError modbusBuildExceptionErr( ModbusSlave *status, uint8_t function, ModbusExceptionCode code, ModbusFrameError parseError )
{
if ( status == (0) ) return MODBUS_ERROR_NULLPTR;
