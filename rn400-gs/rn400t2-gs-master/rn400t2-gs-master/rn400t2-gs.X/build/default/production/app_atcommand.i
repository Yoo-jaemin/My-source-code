...
bool isApply;
char apply_params[32];
char manual_params[4][32];
...
# 37 "app_atcommand.c"
extern float g_last_value[2][LV_TEMP+1];
...
{ ATCRST, "ATCRST", AT_APPLYONLY },

{ ATCBLRST, "ATCBLRST", AT_APPLYONLY },



{ ATCSMODEL, "ATCSMODEL", AT_READAPPLY },
{ ATCFTRST, "ATCFTRST", AT_APPLYONLY },
{ ATCSON, "ATCSON", AT_READAPPLY },


{ ATCISMST, "ATCISMST", AT_READAPPLY },
{ ATCMODCH, "ATCMODCH", AT_READAPPLY },
{ ATCMSTID, "ATCMSTID", AT_READAPPLY },
{ ATCRLYNO, "ATCRLYNO", AT_READAPPLY },
{ ATCPPMMAX, "ATCPPMMAX", AT_READAPPLY },
{ ATCRLYDLY, "ATCRLYDLY", AT_READAPPLY },

{ ATCMODID, "ATCMODID", AT_READAPPLY },
{ ATCMODBR, "ATCMODBR", AT_READAPPLY },
{ ATCMODSW, "ATCMODSW", AT_READAPPLY },
{ ATCMODTO, "ATCMODTO", AT_READAPPLY },


{ ATCCAP, "ATCCAP%", AT_READAPPLY },
{ ATCOFFG, "ATCOFFG%", AT_READAPPLY },
{ ATCOFFT, "ATCOFFT%", AT_READAPPLY },
{ ATCSINM, "ATCSINM%", AT_READAPPLY },
{ ATCSNAME, "ATCSNAME%", AT_STRAPPLY },




{ ATCCAL, "ATCCAL%", AT_READAPPLY },
{ ATCCZR, "ATCCZR%", AT_APPLYONLY },
{ ATCCSP, "ATCCSP%", AT_READAPPLY },

{ ATCMODE, "ATCMODE%", AT_READAPPLY },
...
extern s_Nvm g_nvm;
extern u_Eeprom_Nvm g_eeprmTemp[2];
...
wait_count++;
if (wait_count >= 15) {
memcpy(g_nvm.eeprm[0].byte, g_eeprmTemp[0].byte, 62);
memcpy(g_nvm.eeprm[1].byte, g_eeprmTemp[1].byte, 62);
return 0;
}
}
...
switch (status->type) {
case AT_READONLY: status->isApply = 0; break;
case AT_APPLYONLY: param_copy(status->apply_params, param); status->isApply = 1; break;
case AT_READAPPLY: status->isApply = (param_copy(status->apply_params, param) > 0) ? 1 : 0; break;
case AT_STRAPPLY: status->isApply = (_strcpy(status->apply_params, param) > 0) ? 1 : 0; break;
}
return AT_PARSE_OK;
} else
...
memcpy(g_eeprmTemp[chno].byte, g_nvm.eeprm[chno].byte, 62);
}
if (status->atcommand == ATCFTRST) {
if (g_sens_measure.existChannel[0]) {
memcpy(g_eeprmTemp[0].byte, g_nvm.eeprm[0].byte, 62);
}
if (g_sens_measure.existChannel[1]) {
memcpy(g_eeprmTemp[1].byte, g_nvm.eeprm[1].byte, 62);
}
}
}
...
case ATCMODCH: apply = (uint8_t)_atoi(status->apply_params); if (apply >= 0 && apply <= (2-1)) g_nvm.flash.ctrl_channel = apply; break;
...
case ATCSNAME:
if (_strlen(status->apply_params) < 10)
_strcpy(g_nvm.eeprm[chno].sensor_name, status->apply_params);
else
g_nvm.eeprm[chno].sensor_name[10-1] = '\0';
break;
case ATCCAL:
...
break;
case ATCWIN: apply = (uint8_t)_atoi(status->apply_params); if (apply >= 1 && apply <= 15) g_nvm.eeprm[chno].window_size = apply; break;
...
if (g_sens_measure.isSensorInit[chno] == 1)
{
if (g_last_value[chno][LV_GAS_VOLT] == -999995.0f)
len += _strcpy(&writeBuffer[len], "----");
else
len += _ftoa(g_last_value[chno][LV_GAS_VOLT] , &writeBuffer[len]);
writeBuffer[len++] = ',';
if((g_nvm.eeprm[chno].span_uV - g_nvm.eeprm[chno].baseLine_uV) == 0.0f ||
g_nvm.eeprm[chno].CalibrationGas == 0.0f || g_last_value[chno][LV_GAS_PPM] == -999995.0f)
len += _strcpy(&writeBuffer[len], "----");
else
len += _ftoa(g_last_value[chno][LV_GAS_PPM], &writeBuffer[len]);
writeBuffer[len++] = ',';
if(g_last_value[chno][LV_TEMP] == -999995.0f) {
len += _strcpy(&writeBuffer[len], "----");
} else {
...
if (status->atcommand >= ATCOFFG && status->atcommand <= ATCEXTM) {
if (status->last_update == 0) memcpy(g_nvm.eeprm[chno].byte, g_eeprmTemp[chno].byte, 62);
}
}
...
case ATCVER: len += _strcpy(&writeBuffer[len], "RN400T2-GS""_""5v8"); break;
case ATCUV:
for (i = 0 ; i < 2 ; i++) {
if (g_last_value[chno][i] == -999995.0f)
len += _strcpy(&writeBuffer[len], "----");
else
...
break;
case ATCQ:
len = display_sensor_value(0, chno, len);
break;
case ATCH :
len = display_sensor_value(1, chno, len);
break;
...
case ATCSNAME: g_nvm.eeprm[chno].sensor_name[10-1] = '\0'; len += _strcpy(&writeBuffer[len], g_nvm.eeprm[chno].sensor_name); break;
...
g_atcommand.status.last_update = 0;
g_atcommand.status.atcommand = ATNONE;
g_atcommand.status.isApply = 0;
