...
ModbusError modbusParseRequest0102( ModbusSlave *status, ModbusParser *parser )
{
	...
	//Currently handled data type
		...
		ModbusDataType datatype = parser->_base.function == 1 ? MODBUS_COIL : MODBUS_DISCRETE_INPUT;
	#endif
	...
	#else
		if ( ( parser->_base.function == 1 ? status->coils : status->discreteInputs ) == NULL )
			return modbusBuildExceptionErr( status, parser->_base.function, MODBUS_EXCEP_ILLEGAL_ADDRESS, MODBUS_FERROR_NOSRC );
	#endif

	if ( index >= ( parser->_base.function == 1 ? status->coilCount : status->discreteInputCount ) || \
		(uint32_t) index + (uint32_t) count > (uint32_t) ( parser->_base.function == 1 ? status->coilCount : status->discreteInputCount ) )
	...
	builder->_base.address = status->address;
	builder->_base.function = parser->_base.function;
	...
	for ( i = 0; i < count; i++ )
	{
		...
		#else
			if ( ( coil = modbusMaskRead( parser->_base.function == 1 ? status->coils : status->discreteInputs, \
				modbusBitsToBytes( parser->_base.function == 1 ? status->coilCount : status->discreteInputCount ), i + index ) ) == 255 )
