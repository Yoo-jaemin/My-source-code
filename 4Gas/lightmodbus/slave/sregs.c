...
ModbusError modbusParseRequest0304( ModbusSlave *status, ModbusParser *parser )
{
	...
	#else
		if ( ( parser->_base.function == 3 ? status->registers : status->inputRegisters ) == NULL )
			return modbusBuildExceptionErr( status, parser->_base.function, MODBUS_EXCEP_ILLEGAL_ADDRESS, MODBUS_FERROR_NOSRC );
	#endif

	if ( index >= ( parser->_base.function == 3 ? status->registerCount : status->inputRegisterCount ) || \
		(uint32_t) index + (uint32_t) count > \
		(uint32_t) ( parser->_base.function == 3 ? status->registerCount : status->inputRegisterCount ) )
	{
	...
	#else
		for ( i = 0; i < count; i++ )
			builder->response0304.values[i] = modbusMatchEndian( ( parser->_base.function == 3 ? status->registers : status->inputRegisters )[index + i] );
	#endif
