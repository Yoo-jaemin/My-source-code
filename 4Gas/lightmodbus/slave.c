...
ModbusError modbusBuildException( ModbusSlave *status, uint8_t function, ModbusExceptionCode code )
{
	...
	if ( requestParser != NULL && requestParser->_base.address == 0 )
	{
...
ModbusError modbusParseRequest( ModbusSlave *status )
{
	...
	if ( !functionExec )
	{
		if ( functionMatch ) //Matched but not executed
			err = modbusBuildExceptionErr( status, parser->_base.function, MODBUS_EXCEP_ILLEGAL_FUNCTION, MODBUS_FERROR_NULLFUN ); //User override
		else
			err = modbusBuildExceptionErr( status, parser->_base.function, MODBUS_EXCEP_ILLEGAL_FUNCTION, MODBUS_FERROR_NOFUN ); //No override, no support
	}
...
ModbusError modbusSlaveEnd( ModbusSlave *status )
{
	...
	//Free memory
		...
		free( status->response.frame );
		status->response.frame = NULL;
    #else // by nakholee static memory free
        memset( status->response.frame, 0, LIGHTMODBUS_STATIC_MEM_SLAVE_RESPONSE );
	#endif
...
// nakholee modified
#ifdef LIGHTMODBUS_SLAVE_BASE
ModbusError modbusBuildExceptionErr( ModbusSlave *status, uint8_t function, ModbusExceptionCode code, ModbusFrameError parseError ) //Build an exception and write error to status->parseError
{
	if ( status == NULL ) return MODBUS_ERROR_NULLPTR;
